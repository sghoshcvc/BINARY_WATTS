function BMM = compute_BMM_models(opts,images)
...
for i=1:length(images)
   ...
   % d = d/255;
   %extract LBP features densely
   [d,f] = genericLBP8bit(im,opts.radius,opts.samples);
    
    %% this portion is commeneted now, for spatial position need to be rewritten
    if opts.doMinibox == 0
...
%% Computing global PCA --- PCA is not needed for Binary data
...
for s = 1:length(opts.numSpatialX)
    for j=1:opts.numSpatialX(s)
        for k=1:opts.numSpatialY(s)
            ...
            %d=PCA.eigvec'*d;
            bmm = fitbmdist(d', opts.G, 'Options', struct('MaxIter', 1000, 'TolFun', 1e-4));
...
%u = bmm.Means; 
            ...
            we = bmm.ComponentProportion'; 
            BMM.we = [BMM.we we];
            BMM.mu = [BMM.mu bmm.Means];
            %GMM.sigma = [GMM.sigma sigma];
        end
    end
end
BMM.we = BMM.we/sum(BMM.we);
