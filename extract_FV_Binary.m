function extract_FV_Binary(opts)

%GMM = readBMM(opts.fileGMM);
load(opts.fileBMM);
BMM  = bmdistribution(BMM.mu, BMM.we);
...
for cb=1:nBatches    
    ...
    parfor i=1:length(imagesBatch)        
        ...
        %descrs = descrs / 255;
        descrs =genericLBP8bit(im,opts.radius,opts.samples);
...
%         [descrs,frames] = normalizeSift(opts,descrs,frames);
        ...
        featsBatch(:,i) = single(getImageDescriptorFV(BMM, descrs));        
    end
...
% -------------------------------------------------------------------------
function fv = getImageDescriptorFV(BMM, descrs)
% -------------------------------------------------------------------------
...
% genericLBP8bit(im,opts.radius,opts.samples);
%fv  =  vl_fisher(descrs, GMM.mu, GMM.sigma, GMM.we, 'Improved');
fv = CalFisherVectorBMM(descrs',BMM);
end
